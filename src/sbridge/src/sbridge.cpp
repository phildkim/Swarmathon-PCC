...
// sbridge constructor
sbridge::sbridge(std::string published_name)
{
  ...
  drive_control_subscriber = sNH.subscribe((published_name + "/driveControl"), 10, &sbridge::cmdHandler, this);
  ...
  heartbeat_publisher = sNH.advertise<std_msgs::String>((published_name + "/sbridge/heartbeat"), 1, false);

  // Create a skidsteer publisher to
  skidsteer_publisher = sNH.advertise<geometry_msgs::Twist>((published_name + "/skidsteer"), 10);

  // Create a infoLog publisher to send log information to the log box in the GUI
  info_log_publisher = sNH.advertise<std_msgs::String>("/infoLog", 1, true);
...
void sbridge::cmdHandler(const swarmie_msgs::Skid::ConstPtr& message)
{
  ...
  if (fabs(turn) >= max_turn_rate)
  { // max value needs tuning
  ...
  skidsteer_publisher.publish(velocity);
}
